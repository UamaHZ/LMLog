...
public class PrintToConsole {
    ...
    private void printLine(String tag, String msg, int level) {
        switch (level) {
            ...
            case LMLog.LogLevel.VERBOSE:
                Log.v(tag, msg);
                break;
            case LMLog.LogLevel.DEBUG:
                Log.d(tag, msg);
                break;
            case LMLog.LogLevel.INFO:
                Log.i(tag, msg);
                break;
            case LMLog.LogLevel.WARN:
                Log.w(tag, msg);
                break;
            case LMLog.LogLevel.ERROR:
                Log.e(tag, msg);
                break;
            default:
                break;
    ...
    public void printJson(String tag, String msg, String headString) {
        ...
        try {
            if (msg.startsWith("{")) {
                ...
                message = jsonObject.toString(LMLog.JSON_INDENT);
            } else if (msg.startsWith("[")) {
                JSONArray jsonArray = new JSONArray(msg);
                message = jsonArray.toString(LMLog.JSON_INDENT);
            } else {
                message = msg;
            }
        ...
        printLine(tag, true);
        message = headString + LMLog.LINE_SEPARATOR + message;
        String[] lines = message.split(LMLog.LINE_SEPARATOR);
